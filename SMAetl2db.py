...
#version:		1.0
#author:		West
#Description:	used to prepare SMA daily transaction import
#				Run by each cycle.
#				Data source comes from Excel files under public folder
#Workflow:		read all Excel files based on the cycle date requested,
#				filter all transactions and formatting data
#				import to tblSMA
#
#--------------------------------------------
...
if __name__=="__main__":
	...
	dfcdate = dbq.df_select(driver, db_file, sql)
	latestcycledate = dfcdate.at[(0, 'CycDate')]
	...
	dfsmadata = pd.DataFrame()	#set blank data frame for SMA daily use
	...
	for sma in smalist:
		...
		if not df1.empty:
			dfsmadata = dfsmadata.append(df1, ignore_index=True)
	...
	dfsmadata.columns = labels
	dfsmadata['Client Company Name'] = dfsmadata['Client Company Name'].astype(str).str.replace("'", "")
	dfsmadata['Event Process Date'] = dfsmadata['Event Process Date'].astype(str).str.replace(" 00:00:00", "")
...
#	for row in dfsmadata.to_records(index=False):
...
#		cursor.close()
#	conn.close()
	dbq.add_to_tbl(driver, db_file, table, columns, dfsmadata)
